...
#TODO 8. - Create a new list of the first 3 article's headline and description using list comprehension.
    formatted_news = [{article["title"]:article["description"]} for article in news_list]
    print(formatted_news)
#TODO 9. - Send each article as a separate message via Twilio. 
    for article in formatted_news:
        for (title,desc) in article.items():
            message = client.messages.create(body=f"{COMPANY_NAME} {diff_percent} UP\nHeadline: {title}\n{desc}", from_="+12176263597", to=MY_PHONE)
